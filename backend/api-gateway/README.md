...
- `GET /v1/admin/stats?lookbackMinutes=60&includeRateLimitDetails=true&topN=10&comparePreviousWindow=true&fields=rateLimit,alerts` (admin telemetry: rate-limit saturation, optional sampled top rate-limit keys for abuse triage (`sampledKeys`, `truncated`, `top`), admin action volume, internal call latency/error rollups with p50/p95 + bucket sample counts + `errorByType`, failed-bet reason breakdowns, event publish rates, optional current-vs-previous-window deltas, `alerts` hints for rapid ops triage, and optional top-level field filtering)
