...
- `GET /v1/admin/stats?lookbackMinutes=60&includeRateLimitDetails=true&topN=10` (admin telemetry: rate-limit saturation, optional sampled top rate-limit keys for abuse triage (`sampledKeys`, `truncated`, `top`), admin action volume, internal call latency/error rollups with p50/p95 + bucket sample counts + `errorByType`, failed-bet reason breakdowns, and event publish rates)
