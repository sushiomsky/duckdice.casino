...
- `api-gateway` exposes `GET /v1/admin/stats` for dashboard-ready operational telemetry (rate-limit saturation, optional sampled top-key/IP diagnostics, endpoint request volume rollups, admin/bet activity, failed-bet reason breakdowns, event publish rates, and internal call latency/error rollups with p50/p95 bucketed samples plus compact `errorByType` taxonomies and optional timeout-budget diagnostics via `includeTimeoutDiagnostics=true`; optional previous-window deltas via `comparePreviousWindow=true` including SLO trend comparisons; lightweight `alerts` hints for rapid triage including SLO breach signals with optional severity filtering via `minAlertLevel` and summary metadata; bot-friendly compact `summary`; high-signal `triage` hotspots/recommended actions with optional inclusion of self stats endpoint traffic via `includeSelfEndpoint=true`; objective-based `slo` status/breaches with burn rates and projected budget exhaustion; and optional top-level field filtering via `fields=`).
