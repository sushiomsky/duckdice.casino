...
- `api-gateway` exposes `GET /v1/admin/stats` for dashboard-ready operational telemetry (rate-limit saturation, optional sampled top-key/IP diagnostics, admin/bet activity, failed-bet reason breakdowns, event publish rates, and internal call latency/error rollups with p50/p95 bucketed samples plus compact `errorByType` taxonomies; optional previous-window deltas via `comparePreviousWindow=true`).
